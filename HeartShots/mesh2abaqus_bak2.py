...
def DefineInjectionPoints(mesh):
    ...
    max_radius = 12.0   #10.0 decent
    min_radius = 0.9*max_radius
    disp = 2.5
    ...
    for face in endo_left_faces:
        ...
        if projection>-50.0 and projection<5.0:
        ...
        if len(injection_center)==12: break
    ...
    for node in range(nnodes):
        ...
        for i in injection_center:
            ...
            vector3 = (mesh.nodes[node] - central_point)/radius
            if radius < ref_length: ref_length = radius
        if ref_length < min_radius:
            ratio = ref_length/max_radius
            injection.append(node)
            c_index.append(1.0)
            displacement_x.append(disp*ratio*vector3[0])
            displacement_y.append(disp*ratio*vector3[1])
            displacement_z.append(disp*ratio*vector3[2])
        elif ref_length < max_radius and ref_length > min_radius:
            ratio = ref_length/max_radius
            injection.append(node)
            c_index.append((ref_length-max_radius)/(min_radius-max_radius))
            displacement_x.append(disp*ratio*vector3[0])
            displacement_y.append(disp*ratio*vector3[1])
            displacement_z.append(disp*ratio*vector3[2])
    ...
    disp_x = np.array(displacement_x, dtype=np.float64, copy=True)
    disp_y = np.array(displacement_y, dtype=np.float64, copy=True)
    disp_z = np.array(displacement_z, dtype=np.float64, copy=True)

    total_disp = np.stack((disp_x,disp_y,disp_z), axis=-1)

    return injection, c_index, total_disp

#=============================================================================#
def DefineInfarctedArea(mesh):

    max_radius = 30.0
    min_radius = 0.2*max_radius
    disp = 3.0
    ...
    endo_left_nodes = mesh.left_ventricle_nodes
    endo_left_faces = mesh.left_ventricle_faces
    epi_faces = mesh.epicardium_faces
    face_center = mesh.face_center
    face_normal = mesh.face_normal
    ...
    for face in endo_left_faces:
        ...
        projection = np.dot(face_center[face,:] - left_center,axis_vector)
        if projection>-55.0 and projection<5.0:
            if len(infarct_center)==0:
                infarct_center.append(face)
            else:
                for i in infarct_center:
                    distance = np.linalg.norm(face_center[face,:] - \
                                face_center[i,:])
                    if distance > 1.2*max_radius:
                        add_point = True
                    else:
                        add_point = False
                        break
        ...
        if len(infarct_center)==1: break

    infarct_center = np.array(infarct_center, dtype=np.int64, copy=True)

    infarct_faces = []
    for face in epi_faces:
        for i in infarct_center:
            distance = np.linalg.norm(face_center[face] - face_center[i])
        if distance < max_radius:
            infarct_faces.append(face)

    infarct_faces = np.array(infarct_faces, dtype=np.int64, copy=True)
    ...
    displacement_z = []
    for node in range(nnodes):
        if node in mesh.mitral_nodes or node in mesh.aorta_nodes \
            or node in mesh.pulmonary_nodes or node in mesh.tricuspid_nodes:
            continue
        ref_length = 100.0
        # find the closest face in the infarcted surface
        iface = np.argmin(np.linalg.norm(mesh.nodes[node] - \
                face_center[infarct_faces], axis=1))
        vector3 = face_normal[infarct_faces[iface]]
        face_distance = np.abs(np.dot(mesh.nodes[node] - face_center[infarct_faces[iface]],vector3))
        for i in infarct_center:
            central_point = face_center[i] - 5.0*face_normal[i]
            radius = np.linalg.norm(mesh.nodes[node] - central_point)
            if radius < ref_length: ref_length = radius
        # to create the infacted properties
        if ref_length < 1.2*min_radius:
            infarct.append(node)
            c_index.append(-1.0)
        elif ref_length < 1.2*max_radius and ref_length > 1.2*min_radius:
            infarct.append(node)
            c_index.append(-(ref_length-1.2*max_radius)/(1.2*min_radius-1.2*max_radius))
        # infarcted displacements
        if ref_length < max_radius:
            ratio = face_distance/10.0
            displacement_x.append(disp*ratio*vector3[0])
            displacement_y.append(disp*ratio*vector3[1])
            displacement_z.append(disp*ratio*vector3[2])

    infarct = np.array(infarct, dtype=np.int64, copy=True)
    c_index = np.array(c_index, dtype=np.float64, copy=True)
    disp_x = np.array(displacement_x, dtype=np.float64, copy=True)
    disp_y = np.array(displacement_y, dtype=np.float64, copy=True)
    disp_z = np.array(displacement_z, dtype=np.float64, copy=True)
    ...
    return infarct, c_index, total_disp

#=============================================================================#
def WriteAbaqusInput(mesh, mode="healthy", status="load", field_nodes=None, field_values=None, 
                     field_disp=None):
    ...
    if not infarct and not injection and status=="displacement":
        raise ValueError("Displacement status should not be together with healthy heart.")

    # writing the abaqus input file
    if not injection and not infarct:
        file_out = open("../simulations/heart_healthy.inp", "w")
    elif infarct and status=="load":
        file_out = open("../simulations/heart_infarct_load.inp", "w")
    elif infarct and status=="displacement":
        file_out = open("../simulations/heart_infarct_disp.inp", "w")
    elif injection and status=="load":
        file_out = open("../simulations/heart_injecti_load.inp", "w")
    elif injection and status=="displacement":
        file_out = open("../simulations/heart_injecti_disp.inp", "w")
    ...
    #-----------------------------------------------------------------------------#
    # define predefined fields, this case the infarcted region
    if (infarct or injection) and status=="load":
        file_out.write('**\n')
        ...
        for i in range(field_nodes.shape[0]):
            file_out.write('HEART-1.{}, {}\n'.format(field_nodes[i],field_values[i]))

    #-----------------------------------------------------------------------------#
    # define the displacement step for infarcted region
    if (infarct or injection) and status=="displacement":
        file_out.write('** ----------------------------------------------------------------\n')
        file_out.write('** \n')
        ...
        file_out.write('** Name: injection_exp Type: Displacement/Rotation\n')
        file_out.write('*Boundary\n')
        for i in range(field_nodes.shape[0]):
            file_out.write('HEART-1.{}, 1, 1, {}\n'.format(field_nodes[i],field_disp[i,0]))
            file_out.write('HEART-1.{}, 2, 2, {}\n'.format(field_nodes[i],field_disp[i,1]))
            file_out.write('HEART-1.{}, 3, 3, {}\n'.format(field_nodes[i],field_disp[i,2]))
        file_out.write('** \n')
...
infarct = False                      # True or False
injection = False                   # True or False
status = "load"             # "load" or "displacement"

# initialize infarct and injection parameters
field_nodes = None
field_values = None
field_disp = None
...
# define the anisotropic orientations in the material
mesh.OrientationField()

# define an infarct deseased area
if infarct:
    infarct_area, c0_index, infarct_disp = DefineInfarctedArea(mesh)
    infarct_area += 1

    field_disp = infarct_disp
    # if the status is LOAD, the coordinates of the mesh are updated
    # according to a previous simulation made on DISPLACEMENT mode
    if status == "load" and not injection:
        new_coordinates = np.loadtxt("infarct_disp.dat",dtype=float)
        mesh.nodes += new_coordinates

# define injections if required
# the injections are build as predefined fields in abaqus
if injection:
    # bring the infarcted deformation to the injection case
    old_coordinates = np.loadtxt("infarct_disp.dat",dtype=float)
    mesh.nodes += old_coordinates

    injection_zone, c1_index, injection_disp = DefineInjectionPoints(mesh)
    injection_zone += 1

    if not np.all(injection_zone) or not np.all(c1_index):
        raise ValueError("Injection parameters are still void despite the computations.")

    field_disp = injection_disp
    # if the status is LOAD, the coordinates of the mesh are updated
    # according to a previous simulation made on DISPLACEMENT mode
    if status == "load":
        new_coordinates = np.loadtxt("injection_disp.dat",dtype=float)
        mesh.nodes += new_coordinates

if injection or infarct:
    field_nodes = []
    field_values = []
    if injection:
        for i in range(injection_zone.shape[0]):
            field_nodes.append(injection_zone[i])
            field_values.append(c1_index[i])
    if (infarct and not injection) or (infarct and injection and status=="load"):
        for i in range(infarct_area.shape[0]):
            if infarct_area[i] in field_nodes: continue
            field_nodes.append(infarct_area[i])
            field_values.append(c0_index[i])

    field_nodes = np.array(field_nodes, dtype=np.int64, copy=True)
    field_values = np.array(field_values, dtype=np.float64, copy=True)

# write the abaqus INP file
WriteAbaqusInput(mesh,
                 #mode="infarct",   #"healthy", "injection", "infarct"
                 mode="healthy",   #"healthy", "injection", "infarct"
                 status=status,
                 field_nodes=field_nodes,
                 field_values=field_values, 
                 field_disp=field_disp)
